...
class MInvoice extends CI_Model {
	...
	public function __construct(){
		...
		// function get_invoiceHD($idnoinvoice){
		// 	$data = "SELECT * FROM invoicehd where idinvoicehd = '$idnoinvoice'";
    ...
    function GetDataEimportHD($user){
    	...
    	$data=$this->db->query("SELECT eimporthd.idinvoicehd, eimporthd.noinvoice, eimporthd.tglinvoice, eimporthd.kd_supplier, ms_supplier.nama_supplier
				from eimporthd join ms_supplier on ms_supplier.kd_supplier = eimporthd.kd_supplier WHERE eimporthd.userid='$user'")->row_array();
    	return $data;
    }
    ...
    function GetDataEimport($user,$menu,$noinvoice){
      	...
      	// $this->db->select('eimporthd.noinvoice,eimporthd.tglinvoice,eimporthd.kd_supplier,
        // eimportdt.partno, eimportdt.qty');
      	// $this->db->from('eimporthd');
