...
// end fungsi nomor otomatis
	...
	function get_nopengajuandt1($idsub){
		...
		$data=$this->db->query("SELECT a.id_sub, b.idpengajuan, a.nopengajuan, a.kd_kategori, a.kuota, a.ukuran, a.keterangan, b.tgl_pengajuan, b.status, c.nama_kategori
								FROM pengajuan_dt1 a 
								INNER JOIN pengajuan_hd b ON b.`nopengajuan`= a.`nopengajuan` 
								INNER JOIN ms_kategori c ON c.`kd_kategori`= a.`kd_kategori` 
								WHERE a.`id_sub`=$idsub")->row_array();
		return $data;
	}
	...
	function cekbarang($partno){
		$data = array();
		$data=$this->db->query("select a.nopengajuan, a.status, b.partno
								from pengajuan_hd a 
								inner join pengajuan_dt2 b on a.nopengajuan = b.nopengajuan
								 where b.partno='$partno' and a.status = 'ACTIVE'")->row_array();
		return $data;
	}
	// function cekbarang($idsub,$partno) // hanya cek untuk id dan no pengajuan yg sama saja
	// {
		// 	$data = array();
